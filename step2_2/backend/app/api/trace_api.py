...
@router.get("/evaluation-runs/{run_id}/traces", response_model=list[TraceResponse])
def list_traces(
    run_id: int,
    sample_id: int | None = Query(default=None),
    manager: TraceManager = Depends(get_trace_manager),
):
    return manager.list_traces(run_id=run_id, sample_id=sample_id)
...
@router.get("/evaluation-runs/{run_id}/tool-calls", response_model=list[ToolCallLogResponse])
def list_tool_calls(
    run_id: int,
    sample_id: int | None = Query(default=None),
    manager: TraceManager = Depends(get_trace_manager),
):
    return manager.list_tool_calls(run_id=run_id, sample_id=sample_id)
