...
import { DEFAULT_PROJECT_ID } from '../shared/index.js';
...
function attachChildrenPath(node, parentPath = '') {
...
function upsertNode(nodes, segments, content) {
  const [head, ...rest] = segments;
  const index = nodes.findIndex((node) => node.name === head);

  if (rest.length === 0) {
    const fileNode = { id: `file-${segments.join('-')}`, name: head, type: 'file', content };
    if (index >= 0) {
...
function removeNode(nodes, segments) {
  const [head, ...rest] = segments;
  const index = nodes.findIndex((node) => node.name === head);
...
function renameNode(nodes, segments, nextName) {
  const [head, ...rest] = segments;
  const index = nodes.findIndex((node) => node.name === head);
  if (index < 0) return nodes;
...
export function createWorkspaceManager(options = {}) {
  ...
  async function ensureDirectoryNode(dirPath) {
  ...
  async function renameItem(path, nextName) {
    await ensureWorkspaceDir();
    const normalized = normalizePath(path);
    const segments = normalized.split('/').filter(Boolean);
